// AgentPanel component — spec §7.2, §7.3, §7.4
// Left content area: live streaming output (Screen 1) or PR polling summary (Screen 2)
...
// Estimated rows taken by outer chrome (header + footer) to estimate visible lines
...
export const AgentPanel = defineWidget<AgentPanelProps>((props, ctx) => {
  ...
  // Estimate visible rows for scrollbar
  const titleBarRows = 2;
  const visibleRows = Math.max(1, viewportHeight - CHROME_ROWS - titleBarRows);

  // Content: PR polling summary (Screen 2) or live log (Screen 1)
  ...
  if (sessionComplete) {
    ...
    content = ui.column({ flex: 1, height: 'full', py: 2, px: 2, gap: 1, style: { bg: CONTENT_BG } }, [
      ui.text('──── agent session complete ────', { style: { fg: fg(c.muted) } }),
      ui.text(prText, { style: { fg: fg(c.brand) } }),
      ui.text(
        `⏳ waiting for merge...  ${spin}  (${sessionComplete.elapsed})`,
        { style: { fg: fg(c.accent) } }
      ),
    ]);
  } else {
    // Compute visible window from scrollOffset
    ...
    const lineNodes = visibleLines.map((line, i) =>
      ui.text(
        line.type === 'tool' ? toolLine(line) : line.content,
        { key: String(i), style: { fg: line.type === 'tool' ? fg(c.cyan) : fg(c.text) } }
      )
    );

    content = ui.column(
      { flex: 1, height: 'full', overflow: 'hidden', py: 2, px: 2, gap: 1, style: { bg: CONTENT_BG } },
      lineNodes.length > 0 ? lineNodes : [ui.text('', {})]
    );
  }

  return ui.column(
    { flex: 1, height: 'full', style: { bg: CONTENT_BG } },
    [
      // Title bar
      ui.box(
        {
